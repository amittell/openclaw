...
describe("gateway probe endpoints: shutting-down 503", () => {
  ...
  it("keeps unmarked live probes at 200 while shutting down", async () => {
    // The public contract: external monitors and service managers poll /healthz
    // without the strict marker and must keep seeing 200 during a normal drain.
    // Only the supervised lock-recovery preflight opts into shutdown-aware 503.
    await withGatewayServer({
      prefix: "probe-healthz-public-contract",
      resolvedAuth: AUTH_NONE,
      run: async (server) => {
        markGatewayShuttingDown();
  ...
  it("returns 200 on plain /health even when shutting down (public probe contract)", async () => {
    // Sibling of the /healthz case above: both live probe paths must stay 200
    // for unmarked callers. beta.2 covered both; only /healthz survived the
    // beta.3 rebuild, so a /health regression would have been invisible.
    await withGatewayServer({
  ...
  it("returns the shutting-down phase body on a strict probe, not just the 503", async () => {
    // The status code alone does not tell a supervised lock-recovery preflight
    // WHY the gateway is unavailable; the phase discriminates draining from
    // zombie. beta.2 asserted this body and beta.3 asserts only the code.
    await withGatewayServer({
  ...
  it("resets the shutting-down probe log dedupe on each shutdown cycle", async () => {
    await withGatewayServer({
      ...
      run: async (server) => {
        ...
        // Second probe in the SAME shutdown cycle must not emit again (dedupe).
