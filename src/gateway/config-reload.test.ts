...
describe("startGatewayConfigReloader", () => {
  ...
  it("queues restart-required gateway auth changes in hybrid mode instead of leaving runtime stale", async () => {
    ...
    expect(harness.log.warn).toHaveBeenCalledWith(
      "config reload requires gateway restart; hybrid mode scheduling restart (gateway.auth.token)",
    );
...
describe("startGatewayConfigReloader watcher error recovery", () => {
  ...
  it("keeps hot-reload active when transient errors are separated by working watchers", async () => {
    ...
    try {
      ...
      expect(reloader.hotReloadStatus()).toBe("active");
      // Count only watcher-recovery warnings: unrelated reload warnings (e.g.
