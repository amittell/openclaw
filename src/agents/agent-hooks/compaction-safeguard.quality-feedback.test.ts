...
function wrapQualityFeedback(reasons: string[]): string {
  // The fork routes the defect list through its own wider block; the operator-text block
  // below keeps the 4000-char cap and is anchored separately.
  return wrapUntrustedQualityFeedbackBlock(
...
describe("compaction-safeguard corrective quality feedback", () => {
  it("names only whole identifiers and counts the rest at the largest accepted input", () => {
    ...
    // 10, not upstream's 4: the budget here is the fork's wider feedback block.
    expect(named).toHaveLength(10);
    expect(reasons).toContain("missing_identifiers_omitted:2");
    // Every audited identifier is accounted for: named whole, or counted as omitted.
    expect(named.length + 2).toBe(LONG_IDENTIFIERS.length);
  });
