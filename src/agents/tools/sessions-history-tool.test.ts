...
describe("sessions_history redaction", () => {
  ...
  it("declares complete success and closed error contracts", async () => {
    ...
    expect(compactToolOutputHint(tool.outputSchema)).toBe(
      '{ bytes: number; contentRedacted: boolean; contentTruncated: boolean; droppedMessages: boolean; messages: Array<unknown>; sessionKey: string; truncated: boolean; hasMore?: boolean; nextOffset?: number; offset?: number; pendingInputs?: { items: Array<{ acceptedAt: number; id: string; message: unknown; state: "queued" | "cancelled" | "interrupted"; runId?: string }>; total: number; nextBefore?: number }; returnedCount?: number; sessionLinkRule?: string; shadowedCount?: number; totalMessages?: number } | { error: string; status: "error" | "forbidden" }',
    );
  });
