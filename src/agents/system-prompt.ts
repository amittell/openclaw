...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "When using exec for Python or Node scripts, prefer direct commands like `python path/to/script.py ...` or `node path/to/script.js ...` instead of wrapping them in shell orchestration.",
    "Use the exec tool `workdir` field instead of `cd ... &&` whenever you only need to change directories.",
    "Avoid `echo $?`, pipes, background jobs, `wait`, multiline shell wrappers, or `source ...;` around direct Python/Node exec calls unless the shell behavior itself is the task. If multiple direct script invocations are needed, make separate exec calls.",
    "",
    ...safetySection,
