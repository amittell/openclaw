...
function collectMediaProfileIds(cfg: Awaited<ReturnType<typeof loadModelsConfig>>): Set<string> {
  ...
  for (const agent of cfg.agents?.list ?? []) {
    // SAFETY: optional probe of per-agent tool overrides; ?. yields undefined when the entry has none.
...
export async function modelsAuthCleanCommand(
  ...
  // beta.3 validates a requested agent positionally and throws on an unknown id;
  // an omitted --agent still falls back to the configured default.
  const requestedAgentId = opts.agent?.trim();
  const agentId = requestedAgentId ? resolveKnownAgentId(cfg, requestedAgentId) : defaultAgentId;
