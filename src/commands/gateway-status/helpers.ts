...
export async function resolveAuthForTarget(
  ...
  if (
    ...
    if (interactive.failureReason) {
      return { ...projectResolvedAuth(auth), failureReason: interactive.failureReason };
    }
  }
  return projectResolvedAuth(auth);
}

/**
 * Project only the fields status callers consume. The surface resolver also
 * returns provenance (e.g. `source`) that is not part of this contract, and
 * leaking it changes deep-equality expectations for every caller.
 */
function projectResolvedAuth(resolved: {
  token?: string;
  password?: string;
  diagnostics?: string[];
