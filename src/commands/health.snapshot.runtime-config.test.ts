...
describe("buildRuntimeConfigHealth drift", () => {
  ...
  it("detects drift when secrets.defaults.env repoints the active provider alias", async () => {
    // Watching the `secrets.providers` LEAF covered one of three members, so
    // repointing `secrets.defaults.env` -- which decides the active alias --
    // compared equal and reported a false `ok`. The watched path is the
    // `secrets` PARENT; readConfigPathValue deep-gets and
    // stableHealthValueStringify deep-compares, so the subtree is covered.
    testRuntimeSourceConfig = {
      session: { store: "/tmp/x" },
      secrets: {
        providers: { vault: { kind: "env" } },
        defaults: { env: "OPENAI_API_KEY" },
      },
    };
    testDiskSourceConfig = {
      session: { store: "/tmp/x" },
      secrets: {
        providers: { vault: { kind: "env" } },
        defaults: { env: "OPENAI_API_KEY_ROTATED" },
      },
    };
    testRuntimeConfigSnapshotMetadata = {
      revision: 9,
      fingerprint: "runtime-fingerprint-secrets",
      sourceFingerprint: "live-source-fingerprint-secrets",
      updatedAtMs: 235,
    };
    ...
    expect(runtimeConfig?.state).toBe("drift");
    expect(runtimeConfig?.driftPaths).toEqual(["secrets"]);
  });

  it("detects roster drift on agents.entries when an agent is added on disk", async () => {
    // The roster was watched as `agents.list`, a non-enumerable projection that
    // is absent from sourceConfig and is a REJECTED authored key, so both sides
    // read `undefined` and the roster was never actually watched. `agents.entries`
    // is the authored roster map.
    testRuntimeSourceConfig = {
      session: { store: "/tmp/x" },
      agents: {
        defaults: { model: "openai-codex/gpt-5.5" },
        entries: { main: { model: "openai-codex/gpt-5.5" } },
      },
    };
    testDiskSourceConfig = {
      session: { store: "/tmp/x" },
      agents: {
        ...
        entries: {
          ...
          ops: { model: "openai-codex/gpt-5.5" },
        },
      },
    };
    testRuntimeConfigSnapshotMetadata = {
      revision: 10,
      fingerprint: "runtime-fingerprint-roster",
      sourceFingerprint: "live-source-fingerprint-roster",
      updatedAtMs: 236,
    };
