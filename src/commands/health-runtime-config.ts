...
// Canonical PARENTS, not leaves: readConfigPathValue deep-gets and
// stableHealthValueStringify deep-compares, so a parent covers its subtree.
const RUNTIME_CONFIG_DRIFT_PATHS = [
  "agents.defaults.model",
  "agents.defaults.models",
  // Was "agents.list" -- a non-enumerable projection absent from sourceConfig
  // and a REJECTED authored key, so the roster was never watched at all.
  "agents.entries",
  "models",
  "gateway.auth",
  "auth.profiles",
  "auth.order",
  // Was "secrets.providers" -- that leaf watched 1 of 3 members, so changing
  // secrets.defaults.env reported a false `ok`.
  "secrets",
] as const;
...
function stableHealthValueStringify(value: unknown): string {
  ...
  // SAFETY: null, primitive and array cases all returned above, so this is a plain object.
...
async function readRuntimeConfigDriftState(): Promise<RuntimeConfigDriftState> {
  ...
  try {
    ...
    if (!snapshot.exists) {
      ...
      // SAFETY: reached only when snapshot.exists && snapshot.valid, so it parsed and validated.
