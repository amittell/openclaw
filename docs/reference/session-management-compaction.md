...
- Safeguard mode enables built-in summary quality audits by default. After final budgeting, the retained generated body must contain the required headings, and the exact artifact to be persisted must retain pending asks and exact identifiers. Corrective attempts stay within `qualityGuard.maxRetries`. Exhausting them, or finding that the required facts cannot fit the finalized artifact budget, appends a bounded degraded summary rather than cancelling, logged with `reasonCode=quality_guard_degraded_fallback`; the degraded artifact carries the structured previous summary, the preserved recent turns, and the generated split-turn prefix. A configured compaction provider that throws or returns an empty result falls back to built-in LLM summarization rather than cancelling. A failed built-in corrective generation - including an unresolvable summarization model or missing credentials - still cancels before append and leaves the original transcript authoritative; caller-initiated aborts and transport timeouts are re-thrown. Set `qualityGuard.enabled: false` to skip this behavior. Configured compaction-provider output remains outside the built-in audit loop.
