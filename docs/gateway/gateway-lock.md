...
- Under a service supervisor, a new gateway process that hits either error above first probes `/startupz` on the existing process. The startup lifecycle returns `503` while the predecessor is starting or draining, so the replacement keeps retrying until it can bind. If the existing process reports `200` with `status: "started"`, the new process leaves it in control instead of failing. On systemd, it exits with code `78`; the unit's `RestartPreventExitStatus=78` stops `Restart=always` from looping on a lock or `EADDRINUSE` conflict. If the existing process never reports started, the startup-probe retry is time-bounded and startup then fails with the lock error above instead of looping forever.
