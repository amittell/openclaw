...
- Bot-authored inbound messages are dropped before pipeline entry by default. Set `channels.telegram.allowBots: true` (channel- or account-level) to accept them; accepted bot messages pass through the shared [bot loop protection](/channels/bot-loop-protection) pair guard, which also suppresses channel posts while their originating bot pair is cooling down.
