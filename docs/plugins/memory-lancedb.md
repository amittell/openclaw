...
Agents get four tools from the active memory plugin:
...
- `memory_refresh`: preview similar memories, or atomically replace one by
  `memoryId` (two-phase replace with rollback and an audit log).
