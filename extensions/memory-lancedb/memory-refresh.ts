...
export function registerMemoryRefreshTool(params: {
  ...
  // Embedding config is read per call, not captured, so a config reload between
  // turns takes effect on the next refresh without re-registering the tool.
  resolveCurrentConfig: () => MemoryConfig;
}): void {
  const { api, db, embeddings, resolveEnabledAgentId, resolveRuntimeConfig, resolveCurrentConfig } =
    params;
  api.registerTool(
    (ctx) => {
      ...
      return {
        ...
        async execute(_toolCallId, rawParams) {
          // SAFETY: the runtime validates rawParams against the typebox schema declared above.
          ...
          if (!memoryId) {
            const vector = await embeddings.embed(agentId, text, resolveCurrentConfig().embedding);
