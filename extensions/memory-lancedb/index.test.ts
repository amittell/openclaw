...
describe("memory plugin e2e", () => {
  ...
  test("applies configured embedding timeout/retry bounds only when set", async () => {
    const createLoadLanceDbModule = () => {
      const toArray = vi.fn(async () => []);
      const limit = vi.fn(() => ({ toArray }));
      const vectorSearch = vi.fn(() => createAgentScopedVectorQuery(limit));
      return vi.fn(async () => ({
    ...
    const ensureGlobalUndiciEnvProxyDispatcher = vi.fn();
    const loadLanceDbModule = createLoadLanceDbModule();
    ...
    try {
      // Unconfigured: the OpenAI client carries no operator bounds (SDK
      // defaults apply) and the per-call tool-recall budget is unchanged
      // (15s, maxRetries 0).
      {
        const { default: dynamicMemoryPlugin } = await import("./index.js");
        const registeredTools: any[] = [];
        ...
        await recallTool.execute("test-embed-bounds-unset", { query: "bounds unset" });

        expect(post.mock.calls.at(-1)?.[0]).toBe("/embeddings");
      ...
      // Configured: timeout-less embeds (auto-capture) ride the configured
      // bounds, while per-call recall budgets still win with maxRetries 0.
      {
        const { default: dynamicMemoryPlugin } = await import("./index.js");
        const on = vi.fn();
        const registeredTools: any[] = [];
        ...
        await recallTool.execute("test-embed-bounds-set", { query: "bounds set" });
        expect(post.mock.calls.at(-1)?.[1]).toMatchObject({
          timeout: 15_000,
          maxRetries: 0,
        });
        ...
        const capturePostOptions = post.mock.calls.at(-1)?.[1] ?? {};
        expect(capturePostOptions.timeout).toBe(10_000);
        expect(capturePostOptions.maxRetries).toBe(2);
        expect(post.mock.calls.at(-1)?.[0]).toBe("/embeddings");
      }
    } finally {
  ...
  test("memory_refresh description avoids unqualified 'atomic' framing", async () => {
    ...
    try {
      ...
      const description = refreshTool.description as string;
      ...
      const paramsSchema = refreshTool.parameters as {
