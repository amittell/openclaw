...
function embeddingConfigFingerprint(embedding: EmbeddingConfig): string {
  const { provider, model, apiKey, baseUrl, fallbackBaseUrl, dimensions } = embedding;
  // fallbackBaseUrl participates: createEmbeddings caches one client per fingerprint,
  // so omitting it would keep serving a client that still has the old failover pair.
  return JSON.stringify([provider, model, apiKey, baseUrl, fallbackBaseUrl, dimensions]);
}
...
// splits it across candidates, so a slow endpoint cannot starve the fallback.
...
class OpenAiCompatibleEmbeddings {
  ...
  private clientFor(baseUrl: string | undefined): Promise<OpenAiEmbeddingClient> {
    ...
    if (!client) {
      client = loadOpenAiModule().then(
        ({ default: OpenAI }) =>
          // SAFETY: OpenAiEmbeddingClient is the minimal transport surface this class uses; the SDK client provides post().
  ...
  private async postEmbedding(
    ...
    // cooling down, try them all as a re-probe. Split the caller's timeout across
    // candidates so a primary failure plus a fallback success still fit inside the
    // recall deadline that wraps this call.
    const now = Date.now();
    ...
    for (const baseUrl of candidates) {
      try {
        ...
        // A rejected `dimensions` param is a request-shape problem, not an endpoint
        // outage: surface it so the caller's retry-without-dimensions path runs
        // instead of burning the fallback on the same bad request.
          ...
          direct?.fingerprint === fingerprint
            ...
            : {
                ...
                client: new OpenAiCompatibleEmbeddings(
                  embedding.apiKey,
                  embedding.model,
                  [embedding.baseUrl, embedding.fallbackBaseUrl],
