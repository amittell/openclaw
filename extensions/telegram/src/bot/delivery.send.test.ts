...
function createBot(sendMessage: ReturnType<typeof vi.fn>): Bot {
  // Without `richMessages: true` the send path uses `bot.api.sendMessage` for
  // both the formatted HTML attempt and the plain-text fallback retry.
...
describe("sendTelegramText empty-text silent skip", () => {
  ...
  it("silently skips when Telegram rejects post-strip text with the newer 'text must be non-empty' wording", async () => {
    ...
    // a hard failure that retries forever and pollutes the error log. plainText=""
    // is the delivery contract for chunks with no human-visible source text; a
    // non-empty plain fallback would instead retry the send without formatting.
    const runtime = createRuntime();
    const sendMessage = vi.fn(async () => {
